...
class ControlHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        elif route not in ("/app.js", "/styles.css", "/wxdecipher.css"):
    ...
    def do_POST(self) -> None:
        try:
            ...
            if self.headers.get("Content-Type", "").split(";", 1)[0].strip().lower() != "application/json":
                ...
                return
            payload = self.body()
            if route in {"/api/wechat/decipher/sessions", "/api/wechat/decipher/run", "/api/wechat/decipher/discard", "/api/wechat/decipher/processes", "/api/wechat/decipher/capture-consent", "/api/wechat/export"}:
                if ACTIVE_PROFILE_ID not in {None, "sales-director"}:
