...
class ControlCentreTests(unittest.TestCase):
    def test_a4_json_media_type_is_checked_before_business_dispatch(self):
        routes = {
            "/api/a4/match-play": "handle_match_play",
            "/api/a4/evaluate-signals": "handle_evaluate_signals",
            "/api/a4/recommendations/accept": "handle_accept_recommendation",
            "/api/a4/recommendations/ignore": "handle_ignore_recommendation",
        }
        for route, method in routes.items():
            for content_type in (None, "text/plain;charset=UTF-8", "application/json", "application/json; charset=utf-8"):
                for valid_token in (False, True):
                    with self.subTest(route=route, content_type=content_type, valid_token=valid_token):
                        payload = {"user_input": "synthetic intent", "recommendation_id": "synthetic-rec"}
                        encoded = json.dumps(payload).encode()
                        handler = object.__new__(server.ControlHandler)
                        handler.path, handler.rfile = route, io.BytesIO(encoded)
                        handler.headers = {"X-Director-Token": server.SERVER_TOKEN if valid_token else "invalid-token",
                        ...
                        dispatch = Mock()
                        getattr(dispatch, method).return_value = {"success": True, "data": {"status": "no_match"}}
                        accepted = valid_token and content_type in {"application/json", "application/json; charset=utf-8"}
                        with patch.object(server, "A4_API_HANDLER", dispatch):
                            handler.do_POST()
                        if accepted:
                            self.assertEqual(replies[-1][0], HTTPStatus.OK)
                            getattr(dispatch, method).assert_called_once_with(payload)
                        else:
                            expected = HTTPStatus.FORBIDDEN if not valid_token else HTTPStatus.UNSUPPORTED_MEDIA_TYPE
                            self.assertEqual(replies[-1][0], expected)
                            self.assertEqual(dispatch.mock_calls, [])
    ...
    def test_sales_workbench_uses_guided_forms_and_one_click_weekly_report(self):
        ...
        self.assertIn("结构化副本和索引原文固定保留 7 天", html)
        self.assertIn("输入副本固定保留 30 分钟", html)
        ...
        self.assertIn('id="home-attention"', html)
        self.assertIn('id="customer-filter-query"', html)
        self.assertIn('id="customer-filter-updated"', html)
        self.assertIn('id="customer-list"', html)
        self.assertIn('id="customer-detail"', html)
        self.assertIn('id="customer-tabs"', html)
        self.assertIn('id="quick-customer-context"', html)
        ...
        self.assertIn('function loadMoreCustomerTimeline', javascript)
        self.assertIn('customerState.detailController?.abort()', javascript)
        self.assertIn('customerState.timelineController?.abort()', javascript)
        self.assertIn('if (focus) queueMicrotask', javascript)
        self.assertIn('document.activeElement === focusAnchor', javascript)
        self.assertIn('if (keepDetailFocus)', javascript)
        self.assertIn('id="customer-detail-empty" class="customer-detail-empty" tabindex="-1"', html)
        self.assertIn('customerState.selectedId === selected && customerState.listGeneration === generation', javascript)
        self.assertIn('detailSection("已登记风险", sections.risks', javascript)
        self.assertIn('detailSection("我方行动", sections.actions', javascript)
        self.assertIn('detailSection("正式产出", sections.artifacts', javascript)
        self.assertIn('customerDetailRenderedKey', javascript)
        self.assertIn('/api/accounts/${encodeURIComponent(customerId)}/timeline', javascript)
        self.assertIn('api("/api/attention"', javascript)
