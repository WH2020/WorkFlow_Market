...
description: 创建、修订和检查使用公司规范模板的 Word、Excel 与 PowerPoint 办公文档，并维护本地公司模板库。用于用户上传或指定 DOCX/DOTX、XLSX/XLTX、PPTX/POTX 模板，要求按公司版式生成报告、方案、纪要、表格、周报、汇报材料，或需要登记、选择、激活、替换和核验办公模板时。
---
...
2. 用户上传模板时，先说明模板将保存在本地 Project，不上传第三方；使用管理脚本导入并校验，不直接覆盖原件。
3. 读取模板结构：
   - Word：页面尺寸、页边距、节、样式、页眉页脚、目录和占位符。
   - Excel：工作表、命名区域、公式、数据验证、隐藏内容、打印区域和数字格式。
   - PowerPoint：主题、母版、版式、页面比例、字体、占位符和页脚。
4. 在 `outputs/office/<任务或日期>/` 创建模板副本后填充内容。除非用户要求修改模板库，不在库内文件上生成最终文档。
5. 保持模板的品牌元素、版面层级、字段语义和可编辑结构。缺失事实填“待确认”，不得为填满模板而虚构内容。
6. 按文档类型使用可用的 Documents、Spreadsheets 或 Presentations 工具；开始前加载 Workspace Dependencies。
7. 交付前检查模板匹配、内容完整、公式/链接、分页/溢出、字体、图片、页眉页脚、打印设置和可编辑性。
8. 返回输出路径、模板 ID/版本、已保留结构、已改变内容和待人工确认项。
