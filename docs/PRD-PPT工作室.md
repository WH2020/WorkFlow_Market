...
渲染层不照搬“整页 SVG 作为唯一输出”的路线。默认继续用 `@oai/artifact-tool` 生成原生可编辑的文字、形状、图表和图片；SVG 只作为复杂架构图、流程图或插图的可选页面组件；HTML 只用于快速预览，不作为正式交付文件。这样兼顾可编辑性、Office 兼容性、来源留痕和后续维护。
...
在 task/intent 私有目录使用 `@oai/artifact-tool` 生成 PPTX，并完成逐页渲染、布局、来源备注、页数、文件类型和溢出检查。
...
MVP 继续由主 Agent 负责对话与任务路由，DAG 负责顺序和权限，工具负责确定性读取、冻结、生成和提交。现阶段不把隔离 Subagent 执行器设为前置依赖；未来具备隔离执行器后，可把多来源检索、事实复核和视觉复核拆成有边界的 Subagent，主 Agent 仍负责整合和最终决策。

### 11.3 渲染策略

| 方案 | 定位 | 决策 |
|---|---|---|
| `@oai/artifact-tool` 原生 PPT 元素 | 正式 PPTX | 默认方案，延续当前 QA 与 receipt 链路 |
...
- 复用当前 artifact-tool、Approval、QA、receipt 与恢复链路。
- 先由市场总监完成真实试运行，再开放给产品总监。
...
- 安装隔离 Subagent 执行器后，并行处理来源检索、事实复核和视觉复核。
