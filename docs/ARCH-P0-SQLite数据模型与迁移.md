...
- 状态：schema v1、A1 迁移底座、A2 在线兼容适配/客户只读查询和 A3 客户经营页面已实现；正式数据迁移、A4 建议层和 A5 存储切换未开始
- 适用范围：销售总监本地版
...
- A3 已提供客户列表、客户全景、时间线、今日关注和显式客户上下文，但没有执行正式业务数据迁移或存储切换；A3 今日关注只是查询时确定性投影，不是 A4 持久化信号规则或行动建议。A4 信号建议/Play 和 A5 发布切换仍不能写成已上线能力。
