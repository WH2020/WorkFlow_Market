...
- 状态：设计待评审；A0 门禁已在 Windows x64 通过，macOS 双架构待 CI
- 适用范围：销售总监本地版
...
| 方案 | 优点 | 成本与风险 | 适用判断 |
|---|---|---|---|
| 固定 Node 24.x + `node:sqlite` | 无第三方原生包；安装包更简单；参数化、事务和备份能力足够 | API 仍处于活跃演进；必须固定版本并做跨平台故障测试 | **首选，但必须通过 A0 技术门禁** |
...
阶段 A 先执行 A0 驱动门禁：把打包运行时固定到项目验证过的 Node 24.x 补丁版本，使用 `node:sqlite` 实现 TypeScript `LocalBusinessStore`；Python 工作台仅做只读查询和离线迁移。若门禁任一项失败，则停止进入功能开发，单独评审 `better-sqlite3`，不得在发布时动态切换未验证驱动。
...
- 数据库驱动方案在 A0 门禁完成前仍是待验证设计决策，不能写成已实现能力。
