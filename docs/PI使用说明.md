...
支持 Windows 10/11 与 macOS 13+ 本机部署。先安装 Git、Python 3.11+ 和 Node.js 24.19.0 或更新的 Node 24 版本，再克隆仓库并运行对应平台安装器；安装器负责 pnpm、Pi 0.84.2+、锁定依赖、开源 LibreOffice、本地数据、项目 Package 和环境体检。
...
3. 项目页头提供“上传资料”和“打开数据目录”：上传会自动归入当前项目；数据目录固定为应用安装目录下的 `data/`，无需手动查找路径。
4. 可上传 `.pdf`、`.ofd`、`.docx`、`.xlsx`、`.xls`、`.csv`、`.txt`、`.md`、`.pptx`、常见凭证图片和 `.zip`，单文件最大 32 MiB。文件只写入 `inputs/projects/<project_id>/`，拒绝路径穿透、符号链接和覆盖同名文件。
5. PDF 可直接点击“PDF 入库”，自动带入受控相对路径；其他格式首版只作为任务引用材料，不宣称已完成 Word/Excel 全文解析。

“项目空间”是本地归档与上下文分组，不是多人权限隔离。归档项目会暂停其每日任务，但不会删除历史任务、资料或产物。

### 从邮箱获取报销材料

1. 在“设置 > 报销邮箱”选择邮箱类型，填写邮箱地址、登录账号以及邮箱服务生成的客户端授权码、三方客户端安全密码或应用专用密码。不要填写普通网页登录密码。
2. 工作台会验证 IMAP 连接，并在 Windows 使用当前用户的 DPAPI、在 macOS 使用钥匙串保护凭据。预设邮箱固定使用 TLS 端口 993；自定义服务器默认拒绝本机和私有网地址，除非用户明确允许。
3. 打开“工具栏 > 报销材料”，选择日期范围和可选关键词，点击“搜索报销邮件”。结果只展示主题、发件人、时间和白名单附件，不把邮件正文返回页面。
4. 勾选邮件后点击“导入所选材料”。系统会再次核对邮件身份，将 PDF/OFD、图片、表格、Word 和 ZIP 等允许类型写入独立的 `inputs/reimbursements/<batch_id>/` 报销材料库，并在 `outputs/reimbursements/<batch_id>/` 生成 Markdown 材料清单；导入时不再要求选择项目。
5. 在“工具栏 > 报销材料库”中可打开、重命名、移入项目或删除附件，也可整批移入回收站。删除不会立即销毁文件，可从同页回收站恢复。
6. 项目空间中的普通资料也支持打开、重命名和移入回收站。若文件已被任务或知识记录引用，工作台会显示引用并要求再次确认。

旧版本已经导入项目空间、文件名以 `mail-` 开头的报销附件，可使用“迁移旧材料”复制到独立材料库。迁移会校验文件哈希，项目中的原件不会自动删除。

邮箱始终以只读方式打开，并使用不标记已读的读取命令；不会移动、删除、回复或发送邮件。单次最多选择 20 封邮件、导入 50 个附件，合计不超过 50 MiB；单封邮件超过 20 MiB 会跳过。Outlook/Microsoft 365 需要 OAuth2，首版界面明确禁用，不能用普通密码绕过。重复导入同一附件不会覆盖现有文件；如果同名目标内容不同，整次导入停止。
...
- “查询本地资料”即时匹配项目、任务、来源登记、销售四张台账、项目资料和输出产物，返回受限摘要，不把整个 CSV 行直接送到页面。
...
1. 未切换存储时，保持现有原始资料和 `data/knowledge/source-register.csv` 原位；有效 schema v1 存储指针启用后，以 SQLite `sources/evidence_refs` 为当前知识库。
...
6. 批准后，CSV 模式在同一文件内一次校验并原子替换；SQLite 模式把最多 100 条跨核心表 mutation 和写入回执放入一个短事务。任一记录失败则整批不写。
...
- `knowledge.search` / `knowledge.write`：按严格存储指针检索和带版本更新来源登记；SQLite 模式使用 `sources/evidence_refs`，CSV 模式保留原兼容契约。
- `sales.read` / `sales.write`：一次读取一个或多个销售表；每张表支持查询绑定游标。只允许新增和带版本更新，不允许删除、改主键或写未知字段；SQLite 模式可在一次 Approval 中提交跨核心表事务。
- `account.search` / `account.read_360` / `signals.read`：只读搜索客户、聚合客户关联记录并读取信号；CSV 缺失的关系返回明确空分区，不编造数据。
...
- CSV 兼容模式的事务边界仍是一张文件；有效 schema v1 指针启用后，SQLite 模式支持同一受批载荷中的跨核心表短事务。A5 正式迁移前不会自动切换或双写。
