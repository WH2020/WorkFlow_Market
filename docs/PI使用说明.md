...
支持 Windows 10/11 与 macOS 13+ 本机部署。先安装 Git 和 Python 3.11+，再克隆仓库并运行对应平台安装器。运行时要求 Node.js 24.19.0 或更新的 Node 24 版本；Windows 没有合格版本时，安装器会下载并校验 Node.js 官方免安装包，保存到项目私有 `runtime/node/`，不需要管理员权限。安装器还负责 pnpm、Pi 0.84.2+、锁定依赖、开源 LibreOffice、本地数据、项目 Package 和环境体检。
...
/director-run pdf-import 读取 inputs/某报告.pdf，按页提取证据并准备写入资料库
...
`/director-run` 会创建一个受管任务；同一 Pi 会话同时只运行一个任务。Agent 和 Validator 节点由模型报告完成，逻辑 Tool 节点只能由匹配的适配器完成，Approval 只能由用户推进。行业研究会自动调用一次“公开研究员”，政府合作方案会在初稿后自动调用一次“只读复核员”，用户无需另填表或手工选择角色。若后续要写资料库或销售台账，Agent 必须先冻结完整批次；缺少冻结内容时，状态机不允许进入 Approval。任务状态同时记录在 Pi 会话和 `.pi/director-runtime/tasks/`，重启后可以恢复。普通自然语言请求仍可用于讨论；需要强制流程和审批时应使用 `/director-run` 或本地工作台。
...
每个当前任务都有“任务执行步骤”“AI 处理过程”和消息输入框。执行步骤直接来自同一份受管 DAG，不另建一套待办数据库；它会显示完成数、当前步骤、下一步、前置依赖、阻塞原因、最后更新时间和较长时间未更新提示。调研、政府合作、客户销售、PPT、周报与招投标任务都使用这套显示方式。已完成步骤不能被界面改写，人工确认步骤也不能被单独取消。

步骤右侧的“调整”支持调整当前做法、在该步骤后增加工作、完成后暂停和申请取消；卡片顶部可重新规划全部剩余步骤。这些操作会形成结构化方向请求，排队交给智能核心核对依赖关系，界面不会直接删除 DAG 节点。“申请取消”若碰到必要步骤会被保留并说明原因；“完成后暂停”只是在安全边界等待新指令，绝不会被当作批准。时间线同时显示任务阶段、已执行动作、可公开的判断依据、下一步、审批状态和调整请求送达状态，不显示模型隐藏提示词或逐字思维链。“排队补充”保留当前方向并加入新信息；“调整当前方向”要求 Pi 在当前工具调用结束后、下一次模型调用前重新评估尚未执行的步骤。所有调整都不会修改已完成节点，也不会替代批准、驳回、PPT 大纲修订或重新审批。任务已结束后请使用“再次创建”，不要向历史任务追加消息。

工作台每 3 秒同步任务状态，但会保留当前消息输入框的焦点、选区、光标和内部滚动位置，不会在输入过程中抢走焦点。历史任务默认折叠；“彻底删除”始终显示在卡片标题区，一次确认即可执行。服务端仍要求固定删除确认值，并且只允许删除已结束任务；任务卡、过程、排队消息和演示方案不可恢复，已生成文件、资料库和销售台账不会随卡片删除。
...
这不是操作系统后台服务：应用关闭时不会运行，跨天后不会补做昨天的任务。调度器只负责创建请求，后续仍经过相同 DAG；资料库/销售台账写入、正式 PPT 和任何外发都不会自动批准。
...
PPT 工作室首版的资料范围固定为“公开网页 + 当前销售资料库”：DAG 先执行公开检索与正文读取，再读取内部资料。本地电子文档可直接上传到资料库；PDF 上传后可立即发起按页证据提取。界面不会提供尚未生效的“仅本地/仅公开”开关；不要把秘密、客户口令或未公开项目代号写入会用于公开检索的主题。后续如需这两种范围，必须用权限和节点都独立的受控工作流实现。
...
- “结束任务”由 Pi 执行正式取消。已完成、已结束、已驳回、失败和已替代任务只出现在历史任务区；历史卡片可“再次创建”，也可在折叠状态下“一次确认”彻底删除本地任务记录。删除不会移除已经生成的文件、资料库记录或销售台账变更。
- 同一任务的继续、结束和重新开始都绑定当前 `version`，并与 Pi 共用独占锁；版本变化、活动心跳存在、写入正在提交或已有生命周期请求时会拒绝操作。

## 销售总监资料库

1. 左侧“资料库”统一展示既有来源登记、项目文件、资料库直传文件和生成产物；原件保持原位，不会为了展示而迁移或复制。
2. 点击“上传资料”可直接写入 `inputs/library/<library_id>/versions/`；点击“添加网页”只登记来源，正文未核验前保持“待核验”。
3. 在右侧详情中编辑分类、核验状态、保密级别、项目、客户、商机、投标项目、地区、主题、标签和复核日期。
4. “上传新版本”保留旧文件并把资料重新标记为待核验；完全相同的内容会被拒绝。
5. 资料卡可直接发起客户复盘、政府方案、销售 PPT 或招标解读，任务会自动带入资料编号、原件路径和业务关系。
6. “移入资料库回收站”只从日常检索中隐藏，原始文件和既有来源记录不会被永久删除，可随时恢复。

完整数据边界和验收标准见 [销售总监资料库](销售总监资料库.md)。

## 关联现有资料来源

1. 未切换存储时，保持现有原始资料和 `data/knowledge/source-register.csv` 原位；有效 schema v1 存储指针启用后，以 SQLite `sources/evidence_refs` 为当前来源主库。
...
```text
请先关联现有资料库，列出与“某产品/某客户/某地区”有关的证据、冲突和过期信息，再开始任务。
```
...
1. 在左侧打开“客户经营”，可按关键词、负责人、地区、行业、阶段、健康度和更新时间筛选客户。筛选只影响当前列表，不会改写台账。
2. 选择客户后，顶部和快速输入框会明确显示“当前客户”。客户全景包含概览、时间线、关键人、机会、行动与承诺、信号与风险、资源与资料、证据与任务；缺少的数据会显示空状态，不会自动补写推断。
3. 首页“今日关注”由现有行动、承诺、资源期限、互动时间和已登记信号实时投影；点击卡片可直达相应客户分区。它不会持久化新信号，也不会自动改变客户状态。
4. 从客户页可发起客户复盘、政府合作、行业研究、资源协调/文件和演示文稿。当前客户会预填到任务说明，但任务仍走原 DAG、来源校验和 Approval，不会从页面直接写库或外发。
5. 若要处理另一项工作，可点击顶部客户标签中的“×”清空上下文。上下文只保存在当前应用会话，不会在重启后静默恢复。
6. CSV 兼容模式无法展示尚未结构化的关键人关系、多机会、证据关联和正式写入回执；启用有效 SQLite 指针后才显示这些结构化关系。A5 正式迁移前不要手工创建或切换指针。

开发者可运行 `python scripts/benchmark_customer_operations.py`，使用临时的 10,000 客户和 100,000 条互动数据复核常用查询 P95；脚本不读取或修改正式业务数据。

## 智能招投标

1. 打开左侧“智能招投标”，创建项目并按需关联客户和项目空间。首次只要求项目名称。
2. 进入项目，在底部选择资料类型并上传招标文件、补遗、指定模板或公司材料。上传完成可直接启动解读，项目背景会自动带入任务。
3. 页面根据项目状态推荐下一步，也允许直接选择文件解读、参投决策、应答策划、章节编制、合规复核、生成终稿或结果复盘。
4. AI 生成的投标要求、事实、响应项、章节、检查、风险和决策会先形成可编辑中文卡片。用户可逐条编辑或删除；只有最终批准后才写入 `data/bids/bidding.sqlite3`。
5. 正式标书生成使用读取结果中的 `document_snapshot_sha256` 冻结内容依据，经 Approval 后生成到 `outputs/bids/<bid_id>/`。产物必须通过真实 DOCX 打开、PDF 转换、逐页渲染、空页和溢出检查，并登记文件哈希；重名文件拒绝覆盖。
6. “项目文件”和“正式产物”中的打开按钮会先核对登记路径与 SHA-256，再调用系统默认程序。

当前不自动登录、报名、报价、签章、加密、上传或提交采购平台，不把扫描文件默认上传外部 OCR。更完整说明见 [智能招投标使用说明](智能招投标使用说明.md)。

## 当前限制与边界

- 硬门禁适用于 `/director-run` 或工作台创建的“受管任务”，不是操作系统沙箱；第三方 Pi 扩展和用户在受管任务之外执行的命令仍拥有其自身权限。
- 受管任务运行期间禁用通用 Bash，`data/` 结构化写入不能使用普通 `write` / `edit` 绕过；当前周报和 PPT 工作室具备受控 PPT 生成链路，智能招投标具备受控正式 DOCX 生成链路。通用 Word、Excel 及企业母版自动解析尚未实现。
...
- 微信会话只能通过销售总监工作台主动导入结构化文件，并在用户选择会话、日期且确认当前模型可处理后读取；不连接微信进程、不提取密钥、不修改原库、不接入 WeFlow。原文固定 7 天自动清理。
