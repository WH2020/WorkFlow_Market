...
3. 打开“工具栏 > 报销材料”，选择项目、日期范围和可选关键词，点击“搜索报销邮件”。结果只展示主题、发件人、时间和白名单附件，不把邮件正文返回页面。
4. 勾选邮件后点击“导入所选材料”。系统会再次核对邮件身份，将 PDF/OFD、图片、表格、Word 和 ZIP 等允许类型写入 `inputs/projects/<project_id>/`，并在 `outputs/reimbursements/<project_id>/` 生成 Markdown 材料清单。
