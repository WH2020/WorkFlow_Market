...
class MacOSDistributionTests(unittest.TestCase):
    ...
    def test_setup_installs_the_app_and_writes_a_private_runtime_marker(self):
        ...
        self.assertIn('--ignore-scripts --prod=false', setup)
        self.assertLess(setup.index('scripts/enroll-macos-update.py'), setup.index('--self-test'))
        self.assertIn('PATH=/usr/bin:/bin:/usr/sbin:/sbin', setup)
