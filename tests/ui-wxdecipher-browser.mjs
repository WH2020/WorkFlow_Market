...
try {
  ...
  await page.locator("#wxdecipher-auto-discover").click();
  await page.locator("#wxdecipher-discovery").waitFor({ state: "visible" });
  assert.match(await page.locator("#wxdecipher-discovery-group").inputValue(), /\S+/);
  assert.match(await page.locator("#wxdecipher-discovery-detail").innerText(), /10 分钟/);
  await page.locator("#wxdecipher-discovery-dismiss").click();
  await page.locator("#wxdecipher-discovery").waitFor({ state: "hidden" });
  ...
  const copiedBatch = await readdir(join(fixture.root, "data/wechat/decipher"));
  assert.equal(copiedBatch.length, 1);
  ...
  assert.deepEqual(await readdir(join(fixture.root, "data/wechat/decipher")), copiedBatch);
  ...
  assert.deepEqual(await readdir(join(fixture.root, "data/wechat/decipher")), copiedBatch);
  assert.equal(requests.filter((request) => request.path.endsWith("/upload")).length, 1, "Retry does not recopy");
  ...
  assert.equal(requests.filter((request) => request.path.endsWith("/discover")).length, 1, "Discovery runs only after an explicit click");
