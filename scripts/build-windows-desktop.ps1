...
if (-not $SkipSelfTest) {
    ...
    # Validate the delivered directory, never a temporary copy in the source tree.
    $SelfTest = Start-Process -FilePath $SelfTestPath -ArgumentList "--self-test" -WorkingDirectory $OutputDirectory -WindowStyle Hidden -Wait -PassThru
    if ($SelfTest.ExitCode -ne 0) {
        $LauncherLog = Join-Path $OutputDirectory ".pi\director-runtime\desktop-launcher.log"
