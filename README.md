...
- `sales-director`：客户推进、资源协调、客户与行业研究、政府合作方案、全流程智能招投标、销售文件、复盘和个性化销售行动简报。

用户直接选择销售服务，不需要再选择岗位或理解底层插件。仓库仍保留 Profile 插件框架，供开发者维护既有兼容组合，但销售总监发行版会在界面和 Pi 运行时同时锁定 `sales-director`。

> 销售总监版支持用户主动导入本人有权处理的微信结构化文件或数据库静态副本，可选同快照 WAL 重放及独立媒体恢复。默认不读取微信进程；实验性自动取数据库密钥须另行授权并明确选择本人 Windows x64 微信进程，不提权、不保存密钥、不修改原库，也不接入 WeFlow。当前微信 HTTP 入口仅在 Windows 64 位开放，并要求本机用户隔离及私有目录权限；模型仍只能读取另行确认的会话范围。支持范围及限制见 [微信会话导入与整理](docs/微信会话导入与整理.md)。仓库中原有 WeFlow 代码只为旧版 Codex 工作台兼容保留，不会被桌面发行版加载。
...
也可在“设置 → 模型提供者”把本机 **Claude Code / Codex CLI 选为模型执行后端**，替代工作台 API Key。CLI 负责推理并提出工作台工具调用，Pi 继续执行工具、推进 DAG 和落实审批；已有 API 实例保留。首版只接入官方服务、文字输入和经验证的 CLI 版本，不支持微信会话整理或自定义代理。安装、登录、版本限制与验收说明见 [CLI 模型执行后端](docs/CLI模型执行后端.md)。
...
- [A1 SQLite 存储、迁移与恢复使用说明](docs/A1-SQLite存储与迁移使用说明.md)
- [A2 兼容适配与客户查询使用说明](docs/A2-兼容适配与客户查询使用说明.md)
- [A3 客户经营工作台使用说明](docs/A3-客户经营工作台使用说明.md)
- [个性化销售行动简报](docs/个性化销售行动简报.md)
- [P0 实施任务、测试矩阵与发布门禁](docs/PLAN-P0-实施与验收.md)

阶段 A 的方向是把现有研究、政府合作、资源协调、复盘和 PPT 统一到客户经营上下文中，并以 SQLite 承载可事务化的业务关系。A0 驱动门禁、A1 数据底座、A2 在线兼容适配/客户只读查询和 A3 客户经营工作台已经实现；A4 持久化信号、行动建议与 Play 体验以及 A5 正式数据迁移和切换仍未完成。没有有效 `data/storage-backend.json` 时，现有安装继续以 CSV 为主存储，不会因为发现 `.db` 文件就自动切换。

## Pi 快速开始（Windows / macOS）

正式支持 Windows 10/11 与 macOS 13+ 的本机部署。基础要求为 Python 3.11+ 和 Git；Node.js 要求 24.19.0 或更新的 Node 24 版本。Windows 若没有合格的系统 Node，安装器会从 Node.js 官方站下载固定版本免安装包，核对 SHA-256 后放入项目私有 `runtime/node/`，不需要管理员权限，也不改变其他软件的 Node。安装器还会检查或配置 pnpm 10、Pi 0.84.2+、项目依赖、本地数据和项目级 Pi Package。建议把仓库作为实际工作 Project 使用，以便资料库、台账、模板和输出目录都位于当前工作目录。
...
任务中心会把同一份受管 DAG 显示成“任务执行步骤”，直接给出已完成数量、当前/下一步、前置依赖、阻塞原因、最后更新时间和较长时间未更新提示；不会另外维护一套容易失真的待办状态。步骤菜单支持调整做法、追加工作、设置暂停点、申请取消和重新规划剩余步骤，这些操作只会进入本地受控方向队列，不能直接删除必要节点或替代人工审批。“AI 处理过程”继续展示当前阶段、已执行动作、可公开的判断依据、下一步和用户消息状态，而不是模型隐藏的逐字思维链。运行期间也可以“排队补充”信息或“调整当前方向”；消息由 Pi 在当前工具调用结束后、下一次模型调用前接收，不能绕过已完成节点、冻结载荷、权限边界或人工审批。
...
每次发起任务前，可以在工作台顶栏选择“本次模型”和“思考强度”。选择会随请求冻结，Pi 在接手任务、发出第一条任务提示前实际切换模型；最终生效的模型和思考等级会写入任务记录并显示在任务卡。API 模型不支持所选等级时，以 Pi 实际裁剪后的等级为准；Codex CLI 自动读取模型目录并联动支持的档位，不支持的值直接拒绝，不静默裁剪。历史任务“再次创建”、中断任务“重新开始”和每日定时任务都会保留各自的显式选择；选择“默认”时，Codex 使用该模型保存的强度，其他模型恢复应用启动时的默认设置。详见 [CLI 模型执行后端](docs/CLI模型执行后端.md)。
...
左侧“检查更新”和“设置 → 应用更新”可检查 GitHub Releases 正式版本，默认约每 5 分钟检查，可关闭自动检查。当前未发布源码支持 Windows 安装版的一键程序更新，以及 macOS 通用安装版的 `.app` 与程序代码同步更新：用户确认后下载、验签、停机、替换、试启动，失败恢复；配置、业务数据和安装根目录保持不变。Mac 首版保留已登记的 Python/Node/第三方依赖，依赖变化时提示手动安装。Windows 已在全新合成安装中完成真实桌面升级和跨启动恢复；Mac 已做本地代码验证，仍待 Apple Silicon / Intel 实机验收。现有公开版本仍须先手动安装以后正式发布的更新器版本。自动检查不会自行安装；更新备份暂不自动清理。详见 [Windows 一键更新](docs/Windows一键更新.md)、[macOS 一键更新](docs/macOS一键更新.md)及[更新检测历史说明](docs/应用更新.md)。
...
正常使用请直接双击 `Agent4Market.exe`。桌面窗口首页按“待确认、进行中、本周完成”组织工作，并提供销售快捷指令、项目空间、每日定时任务和自定义操作；没有市场总监或产品总监入口。开发模式下的 HTTP 服务仍只监听本机，不会自动发送文件。完整说明见 [Pi 使用说明](docs/PI使用说明.md)。

### 项目空间、每日任务与搜索

- 项目空间把任务、资料和产物归到具体客户或机会；未归类的历史任务进入“日常工作”。上传资料只接受 PDF/OFD、Word、Excel、CSV、文本、Markdown、PPTX、常见凭证图片和 ZIP，单文件最大 32 MiB，同名文件不会被覆盖。当前自动提取能力以 PDF 为主，其他文件可作为任务引用材料。
...
左侧“资料库”把来源登记、项目文件、直接上传文件和生成产物合并为一个销售工作入口，按客户、商机、政府合作、招投标、行业、销售资产、公司能力和内部资源分类。用户可以编辑资料名称、核验状态、保密级别、标签、复核日期及业务关联，并从资料卡直接发起客户复盘、政府方案、销售 PPT 或招标解读。

资料库采用“原件不动、统一索引”的方式：既有来源和项目文件不迁移、不复制；直接上传到资料库的新文件保存在 `inputs/library/<资料编号>/versions/`；索引、关系、版本和可恢复归档状态保存在 `data/library/catalog.json`。移入回收站只从日常检索中隐藏资料，不永久删除原件。真实目录与文件均被 Git 忽略，公开仓库只保留 `data/library/catalog.example.json`。

底层兼容层仍使用 `shared.knowledge`、`knowledge.search` / `knowledge.write`、`sources/evidence_refs` 和 `/api/knowledge` 等既有标识，以保证原有 CSV/SQLite 数据和审批链不失效。受控适配器严格按 `data/storage-backend.json` 选择来源与销售存储：指针不存在时继续使用现有 CSV；有效 schema v1 指针启用 SQLite；指针损坏、越界、版本不支持或数据库不可用时直接停止，绝不静默回退。网页或电子文档读取形成的来源 URL 与精确入库 mutation 会按任务持久化到 `.pi/director-runtime/evidence/`。结构化写入仍需冻结完整批次、SHA-256 和存储绑定并由用户批准。完整说明见 [销售总监资料库](docs/销售总监资料库.md)。
...
它仍不是无人值守或生产级编排平台：没有云端多租户、自动外发、通用 Word/Excel 文件适配器或企业母版自动解析。A3 已提供今日关注、客户筛选列表、显式客户上下文、客户 360、证据化时间线和五类快捷任务入口；这些页面只读取当前 CSV/SQLite 后端，任何业务变更仍创建受管任务并经过 Approval。正式业务数据尚未迁移或切换，持久化信号规则、行动建议和 Play 推荐属于 A4。免密公开检索依赖第三方共享额度，不承诺持续可用；任何搜索摘要、正文、PDF 与 Subagent 输出都视为不可信资料，不能越过现有来源校验和人工审批。
