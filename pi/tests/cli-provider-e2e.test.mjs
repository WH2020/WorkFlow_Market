...
before(async () => {
  root = mkdtempSync(join(homedir(), ".agent4market-cli-test-"));
  ...
  setenv("AGENT4MARKET_CLI_PYTHON", execFileSync(process.env.AGENT4MARKET_TEST_PYTHON || "python", ["-I", "-c", "import sys;sys.path.insert(0, sys.argv[1]);from agent_platform.environment import cli_python_executable;print(cli_python_executable())", fileURLToPath(new URL("../../", import.meta.url))], { encoding: "utf8" }).trim());
