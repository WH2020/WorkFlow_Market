...
6. `build_payload` 只编制下述精确载荷，不生成文件。载荷必须原样携带当前快照的 `snapshot_sha256`；Profile、period、URL 以及本地来源路径/文件 SHA-256 都必须来自该快照。进入 `validate_payload` 后，调用 `director_propose_write_intent(logical_tool="artifact.deck.write", payload=完整载荷)` 做确定性绑定校验并冻结载荷 SHA-256，再完成 Validator。
7. 到达 Approval 必须暂停。用户批准的是完整载荷及其 SHA-256；批准前不得创建 `outputs/*.pptx`，载荷变化后必须重新冻结、重新批准。
8. 批准后 `render_deck` 才调用 `director_artifact_deck_write`。适配器使用 `@oai/artifact-tool` 在当前 task/intent 的私有目录生成可编辑 PPT，逐页生成 PNG 和 layout JSON，并运行官方 `slides_test.py`；QA 通过后才独占提交到 `outputs/` 并写入 receipt。不得用普通 `write/edit` 伪造 `.pptx`。
9. 每页外部事实和外部资产都放入 speaker notes 的 `[Sources]` 块；本地来源写项目相对路径，PDF 来源同时记录页码。URL 不得包含账号、口令、token、签名或密钥查询参数。
10. 生成后仍不发送、不上传、不对外使用；外部使用由用户另行决定。

## 适配器载荷

- `schema_version` 固定为 `1.0`。
- `snapshot_sha256` 必须是当前 `director_weekly_snapshot` 返回值，不得复用另一任务或另一周期的哈希。
- `output_name` 是安全 ASCII `.pptx` 文件名；不得包含路径，已有文件不得覆盖。
- `profile_id` 使用当前 Profile；`template_id` 第一版固定为 `ceo-weekly`。
